...
- Provider rejections of content already in the history (an image whose base64 does not decode, for example) map to `LLMInvalidToolResultContentError` and recover through the same `state.rebuild_view()` + `CondensationRequest()` path as malformed history; with no condenser the error is re-raised. Undecodable `data:` image URLs are also dropped and replaced with a text placeholder in `openhands-sdk/openhands/sdk/llm/utils/image_validation.py` before the request leaves the SDK, so a single corrupt screenshot cannot wedge a conversation.
