...
- SDK tool-call compatibility shims in `openhands-sdk/openhands/sdk/agent/utils.py` intentionally keep terminal fallback narrow: only exact bare command names like `ls`, `pwd`, and `find` may map to `terminal`, while malformed `grep` payloads should fall back to the normal missing-tool error path instead of executing a shell command.
