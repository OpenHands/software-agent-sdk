...
- In `.github/run-eval/resolve_model_config.py`, mark eval models with `disable_vision: True` when the provider rejects the remote image URLs used by swebenchmultimodal even if LiteLLM advertises vision support; `moonshot/kimi-k2.6` needs this override.
