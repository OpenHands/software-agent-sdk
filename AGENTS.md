...
- Dependabot's `github-actions` ecosystem is enabled in `.github/dependabot.yml`, but Python (`pip`/`uv`) ecosystems are intentionally NOT enabled because Dependabot does not yet fully support uv workspaces (this is a multi-package monorepo with a single root `uv.lock`). As of the July 2026 recheck the upstream blockers are still open/unfixed: `dependabot-core#14004` (resolves workspace members on PyPI), `dependabot-core#14103` (member deps ignored; fix PR #14627 doesn't work in practice), `dependabot-core#13202` (no `dependency-type` grouping), `dependabot-core#14665` (grouped security updates drop packages on case mismatch), `dependabot-core#14119` (version updates fail on `pyproject.toml`/`uv.lock` casing mismatch). Re-enable Python updates only after those blockers are resolved; see issue #2510.
