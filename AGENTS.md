...
- Agent-server startup prewarms the user + public profile skill catalog before readiness, in parallel with other stateless preload work, so profile-launched conversations do not pay the initial OpenHands/extensions clone cost. `Config.prewarm_profile_skills` (env `OH_PREWARM_PROFILE_SKILLS`) defaults to true; disable it for offline deployments. The prewarm runs via `asyncio.to_thread` and failure is non-fatal.
