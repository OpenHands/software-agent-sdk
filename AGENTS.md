...
- The uv lock security scanner in `.github/scripts/scan_uv_lock_security.py` now treats Python startup hooks (`.pth`, `sitecustomize.py`, `usercustomize.py`) and dependency graph deltas as first-class review signals, extracts `Requires-Dist` names from wheel/sdist metadata to compare artifact-declared dependencies with the lock entry, reports deterministic old-vs-new archive member deltas for changed lock entries, and inspects a representative platform wheel plus a manual follow-up checklist when packages ship many wheels.
