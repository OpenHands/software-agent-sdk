...
- Provider rejections that cite a message offset (`messages.<N>.content...`) indict content already persisted in the history, so retrying reproduces them exactly. They map to `LLMHistoryContentRejectedError` and are recovered in the `LocalConversation` run loop (`run()`/`arun()`) by rolling the offending turn out of the active branch via `navigate_to` and injecting an explanation — not in `Agent.step()`, which cannot move HEAD. A turn is every `ActionEvent` sharing an `llm_response_id`; roll back the whole group or a `tool_use` is orphaned from its `tool_result`. Recovery is one-shot per run (`_history_rollback_used`). Configuration bad requests must stay out of this path: discarding history never fixes them.
- Complementary to the rollback: `openhands-sdk/openhands/sdk/llm/utils/image_validation.py` runs a prevention pass inside `LLM._begin_chat_messages` and `LLM._prepare_responses_messages` that swaps `data:` image URLs with an undecodable base64 payload for a text placeholder before the request goes out. It sits under `LLM.drop_undecodable_images` (default True). When it fires, rollback simply does not need to for that vector — prevention keeps the turn's other work while rollback would discard it. Keep the two layers independent: prevention runs on the way out, rollback catches whatever leaks through.
