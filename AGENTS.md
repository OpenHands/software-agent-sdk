...
- The `/api/llm/models/verified` response should return OpenHands-provider entries as fully qualified `openhands/<model>` identifiers rather than bare upstream model ids; otherwise downstream consumers can mistakenly treat any provider-verified model id as OpenHands-verified.
