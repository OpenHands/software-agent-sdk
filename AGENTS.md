...
- As of April 2026, keep `.github/dependabot.yml` limited to `github-actions`; Dependabot's native `uv` ecosystem still has open blockers for this workspace monorepo, especially member package dependencies not being updated (`dependabot-core#14103`), workspace members being treated as external deps (`#14004`), ignored `dependency-type` groups for `uv` (`#13202`), and grouped security updates silently dropping packages (`#14665`).
