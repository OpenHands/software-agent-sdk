...
- Moonshot/Kimi multimodal requests reject remote `image_url` values like GitHub-hosted screenshots with `unsupported image url`; keep Moonshot vision enabled, but inline remote image URLs as `data:` URLs before chat/responses transport instead of disabling vision in eval config.
