...
- Secret serialization uses `serialize_secret()` from `openhands-sdk/openhands/sdk/utils/pydantic_secrets.py`. It handles cipher encryption, `expose_secrets=True`, and default Pydantic masking. For `dict[str, str]` fields whose values are all secrets, wrap each value in `SecretStr` and call `serialize_secret` per value (see `LookupSecret._serialize_secrets` and `ACPAgent._serialize_acp_env` for examples). Do not hand-roll redaction logic in field serializers.
