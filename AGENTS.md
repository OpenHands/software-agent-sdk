...
- Secret serialization utilities live in `openhands-sdk/openhands/sdk/utils/pydantic_secrets.py`. Use `serialize_secret()` for individual `SecretStr` fields and `serialize_secrets_dict()` for `dict[str, str]` fields whose values are all secrets. Both follow the same contract: cipher → encrypt, `expose_secrets=True` → real values, default → `REDACTED_SECRET_VALUE`. Do not hand-roll redaction logic in field serializers; always delegate to these shared helpers.
