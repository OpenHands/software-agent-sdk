...
def check_repl_examples(docstring: str, name: str, lineno: int, file: Path) -> list[Violation]:
...
def check_unfenced_shell_config(docstring: str, name: str, lineno: int, file: Path) -> list[Violation]:
                ...
                if stripped.startswith("# "):
                    violations.append(
                        Violation(
                            ...
                            message=(
                                "Shell/config examples with # comments should be in "
                                "```bash code blocks. Otherwise # becomes a markdown header."
                            ),
                        )
                    )
                    # Only report once per docstring
...
def check_docstring(docstring: str, name: str, lineno: int, file: Path) -> list[Violation]:
...
def main() -> int:
    ...
    if warning_violations:
        print(f"\n⚠️  Found {len(warning_violations)} docstring warning(s) in non-core files:\n")
    ...
    if strict_violations:
        print(f"\n❌ Found {len(strict_violations)} docstring error(s) in core API files:\n")
    ...
    if warning_violations:
        print(f"✅ Core API files pass. {len(warning_violations)} warnings in other files.")
    else:
