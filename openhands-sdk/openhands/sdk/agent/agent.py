...
class Agent(AgentBase):
    ...
    def current_view(self, conversation: LocalConversation) -> list[str]:
        ...
        using the condenser if available to provide a condensed view.
        ...
        # If a condenser is available, use it to get the condensed view
        if self.condenser is not None:
            view = View.from_events(state.events)
            condensation_result = self.condenser.condense(view)
            ...
            if isinstance(condensation_result, View):
                llm_events = condensation_result.events
            else:
                # If condenser returns a Condensation, fall back to base implementation
                return super().current_view(conversation)
        else:
            # No condenser, use base implementation
            return super().current_view(conversation)
