...
def _extract_exec_segments(action: ActionEvent) -> list[str]:
    ...
    when an earlier field is adversarially large (issue #2706). tool_name has
    no length validation anywhere in the SDK; a 30K hallucinated name would
    otherwise consume the full budget and hide the arguments payload.
    """
    segments: list[str] = []
    total = 0

    def _add(text: str) -> None:
        """Append text to segments, tracking joined-length budget.

        The nonlocal `total` tracks the length of the eventual
        ``" ".join(segments)`` string, so each call charges one char for
        the space separator that will precede the segment (except the
        first). This guarantees ``len(" ".join(segments)) <=
        _EXTRACT_HARD_CAP`` regardless of how many short segments are
        produced (a JSON object with many single-char leaves would
        otherwise inflate the joined length via separators).

        Silently skips if no budget remains.
        """
        nonlocal total
        separator_len = 1 if segments else 0
        remaining = _EXTRACT_HARD_CAP - total - separator_len
        if remaining <= 0:
            return
        if len(text) > remaining:
            text = text[:remaining]
        segments.append(text)
        total += len(text) + separator_len
