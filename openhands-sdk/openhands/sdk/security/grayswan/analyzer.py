...
from openhands.sdk.event import ActionEvent, Event, LLMConvertibleEvent
...
class GraySwanAnalyzer(SecurityAnalyzerBase):
    ...
    low_threshold: float = Field(
        default=0.3,
        description="Risk threshold for LOW classification",
    )
    medium_threshold: float = Field(
        default=0.7,
        description="Risk threshold for MEDIUM classification",
    )
    high_threshold: float = Field(
        default=1.0,
        description="Risk threshold for HIGH classification",
    )
    api_url: str = Field(
        ...
        if self._client is None or self._client.is_closed:
            self._client = httpx.Client(
                ...
                headers={
                    ...
                    "x-grayswan-api-key": self.api_key or "",
    ...
    def security_risk(self, action: ActionEvent) -> SecurityRisk:
        ...
        try:
            ...
            recent_events: list[LLMConvertibleEvent] = []
            ...
            events_to_process: list[LLMConvertibleEvent] = recent_events + [action]
