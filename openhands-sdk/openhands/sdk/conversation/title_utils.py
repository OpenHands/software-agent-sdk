...
# Conservative peel of a leading 思考 reasoning block. Some providers
# (e.g. Qwen3 behind Nebius) return reasoning inline in `content` as
# `思考reasoning…思考` instead of the normalized `reasoning_content`
# field, which would otherwise leak into the autogenerated title.
# The peel only targets a leading block so mid-text literal `思考`
# occurrences in a legitimate title are preserved.
_THINK_BLOCK_RE = re.compile(r"^\s*思考[\s\S]*?思考\s*", re.DOTALL)


def _strip_leading_think_block(text: str) -> str:
    """Remove a leading ``思考`` block from ``text`` if present.

    The conservative peel handles two shapes:

    * A closed block ``思考…思考`` at the start of the response is
      stripped and the remainder is returned.
    * An unterminated block (the response begins with ``思考`` and
      never closes) consumes the entire response, returning ``""`` so
      the LLM title path yields no title and the existing
      user-message truncation fallback runs.

    A leading ``思考`` substring that is NOT a complete unclosed block
    (e.g. a mid-text literal occurrence in a legitimate title) is
    preserved unchanged.
    """
    if not text:
        return ""
    if not text.lstrip().startswith("思考"):
        return text
    match = _THINK_BLOCK_RE.match(text)
    if match:
        return text[match.end() :]
    # Leading 思考 with no closer — the whole response is the think
    # block, so consume it and let the caller fall back.
    return ""
...
def generate_title_with_llm(
    ...
    try:
        ...
        if response.message.content and isinstance(
            ...
            title = _strip_leading_think_block(response.message.content[0].text).strip()

            # A response that contained only a 思考 block yields no
            # usable title from the LLM path; fall back to the existing
            # user-message truncation rather than handing back the
            # peel whitespace.
            if not title:
                logger.warning(
                    "LLM title response contained only a 思考 block; "
