...
class LocalConversation(BaseConversation):
    ...
    def generate_title(self, llm: LLM | None = None, max_length: int = 50) -> str:
        ...
        Title generation is decoupled from the agent's LLM. If an LLM is provided,
        it will be used to generate a descriptive title. If no LLM is provided,
        the title is generated by truncating the first user message.

        Args:
            llm: Optional LLM to use for title generation. If not provided,
                 falls back to simple message truncation.
