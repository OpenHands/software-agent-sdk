...
class RemoteWorkspace(RemoteWorkspaceMixin, BaseWorkspace):
    ...
    def get_llm(self, **llm_kwargs: Any) -> "LLM":
        ...
        to retrieve LLM configuration (model, api_key, base_url) from the
        agent-server's persisted settings.

        Note:
            This method uses ``plaintext`` mode which is intended for backend
            clients only. The raw API key is returned in the response and used
            to construct the LLM instance locally.
        ...
        response = self.client.get(
            _SETTINGS_API_BASE, headers={"X-Expose-Secrets": "plaintext"}
        )
        response.raise_for_status()
        data = response.json()
        ...
        # Caller kwargs override fetched settings
        kwargs.update(llm_kwargs)

        # Warn if no API key is configured (likely to fail)
    ...
    def _transform_mcp_config_to_servers(
        ...
        for stdio_server in mcp_config_data.get("stdio_servers") or []:
            ...
            # STDIO servers have an explicit name field - check for collision
