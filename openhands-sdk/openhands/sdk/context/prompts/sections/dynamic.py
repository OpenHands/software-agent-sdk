...
class CustomSecretsSection:
    ...
    def render(self, ctx: PromptContext) -> str | None:
        ...
        return (
            ...
            "* Avoid exposing raw secrets: Never echo or print the full value of secrets (e.g., avoid `echo $SECRET`). The conversation history may be logged or shared, and exposing raw secret values could compromise security. Instead, use secrets directly in commands where they serve their intended purpose (e.g., in curl headers).\n"
            "* Refreshing expired secrets: Some secrets (like GITHUB_TOKEN) may be updated periodically or expire over time. If a secret stops working (e.g., authentication failures), try using it again in a new command - the system should automatically use the refreshed value. For git operations, never put a secret directly in a remote URL or command argument: it gets written to `.git/config` in plaintext and stays there even after the value is masked from your terminal output. Run `gh auth setup-git` once instead (with the token available as GITHUB_TOKEN/GH_TOKEN) to configure a credential helper, then use `git push`/`git pull`/`git fetch` normally.\n"
