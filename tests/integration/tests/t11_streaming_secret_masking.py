"""Test that a registered secret never reaches stream deltas or the trajectory.
...
class StreamingSecretMaskingTest(BaseIntegrationTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.persistence_base = tempfile.mkdtemp(prefix="streaming-secret-")
    ...
    def verify_result(self) -> TestResult:
        ...
        # The user's own instruction carries the value, so only events the
        # agent or its tools produced must be free of it on disk.
        ...
        persisted = [n for n in sorted(os.listdir(events_dir)) if n.endswith(".json")]
        for name in persisted:
            with open(os.path.join(events_dir, name)) as f:
                raw = f.read()
            if SECRET_VALUE in raw and json.loads(raw).get("source") != "user":
                return TestResult(
                    success=False, reason=f"raw secret persisted in {name}"
                )
        ...
        return TestResult(
            success=True,
            reason=(
                f"secret masked across {len(deltas)} delta(s) and absent from "
                f"{len(persisted)} persisted event file(s)"
            ),
        )
