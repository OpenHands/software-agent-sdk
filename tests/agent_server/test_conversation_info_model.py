...
def test_conversation_info_from_sources_maps_state_fields():
    ...
    state_fields = set(ConversationInfo.STATE_FIELDS)
    assert info.model_dump(mode="json", include=state_fields) == (
        state.model_dump(mode="json", include=state_fields)
    )
...
def test_conversation_info_classifies_every_conversation_state_field():
