...
def test_openapi_schema_no_duplicate_titles():
    ...
    # 2. Module-qualified names from different modules
    #    (e.g., mcp__types__Tool, openhands__sdk__tool__spec__Tool)
    duplicates = {}
    for title, names in title_to_names.items():
        if len(names) > 1:
            ...
            # Check if duplicates include module-qualified names
            # (double underscores indicate module paths)
    ...
    assert not duplicates, (
        ...
        "(Input/Output pairs and module-qualified names are expected)."
    )
