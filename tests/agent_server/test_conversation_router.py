...
def test_start_conversation_launch_tool_conflict_returns_409(
    client, mock_conversation_service
):
    ...
    mock_conversation_service.start_conversation.side_effect = (
        AgentLaunchToolConflictError(
            "Launch tool 'canvas_ui' conflicts with the resolved agent"
        )
    )
    ...
    try:
        ...
        assert response.status_code == 409
        assert "conflicts with the resolved agent" in response.json()["detail"]
    finally:
        client.app.dependency_overrides.clear()
