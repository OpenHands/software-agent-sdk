...
# ---------------------------------------------------------------------------
# Litellm instrumentation must be disabled to keep the SDK's streaming path
# working. lmnr's litellm wrapper replaces the return value of
# ``litellm.acompletion(stream=True)`` with its own async generator, which
# breaks the SDK's ``CustomStreamWrapper`` contract (issue #3972).
# ---------------------------------------------------------------------------


def _call_maybe_init_laminar():
    """Run ``maybe_init_laminar`` with lmnr patched and return the init kwargs."""
    from openhands.sdk.observability.laminar import maybe_init_laminar
    ...
    with patch("lmnr.Laminar") as mock_laminar:
        with patch("lmnr.LaminarLiteLLMCallback"):
            with patch("litellm.callbacks", new=MagicMock()):
                mock_laminar.is_initialized.return_value = False
                maybe_init_laminar()
                return mock_laminar.initialize.call_args.kwargs


@pytest.fixture
def _lmnr_env(monkeypatch):
    """Temporarily set/unset the env vars ``maybe_init_laminar`` inspects."""
    for key in ("LMNR_PROJECT_API_KEY", "LMNR_BASE_URL", "LMNR_FORCE_HTTP"):
        monkeypatch.delenv(key, raising=False)
    yield monkeypatch


def test_litellm_disabled_when_using_laminar_backend(_lmnr_env):
    """Laminar's litellm wrapper must be disabled so SDK streaming still works."""
    _lmnr_env.setenv("LMNR_PROJECT_API_KEY", "test-key")

    init_kwargs = _call_maybe_init_laminar()
    disabled = init_kwargs["disabled_instruments"]
    from lmnr import Instruments

    assert Instruments.LITELLM in disabled


def test_litellm_disabled_when_using_otel_backend(_lmnr_env):
    """OTEL-only backends must also exclude lmnr's litellm wrapper."""
    _lmnr_env.setenv("OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", "http://otel:4317")
    ...
    assert Instruments.LITELLM in disabled
    # And the existing browser-instrument exclusions still apply.
    assert Instruments.BROWSER_USE_SESSION in disabled
    assert Instruments.PATCHRIGHT in disabled
    assert Instruments.PLAYWRIGHT in disabled
