...
def test_running_execution_status_becomes_error_on_restart():
    """Test that a conversation with RUNNING status becomes ERROR when resumed.

    If a conversation is serialized while in RUNNING state, it indicates the
    process crashed or was terminated unexpectedly. On restart, such conversations
    should have their execution_status set to ERROR rather than remaining RUNNING.
    """
    with tempfile.TemporaryDirectory() as temp_dir:
        llm = LLM(
            model="gpt-4o-mini", api_key=SecretStr("test-key"), usage_id="test-llm"
        ...
        # Set execution_status to RUNNING (simulating crash during execution)
        ...
        # Verify the RUNNING status was persisted
        base_state_path = Path(persist_path_for_state) / "base_state.json"
        persisted_data = json.loads(base_state_path.read_text())
        assert persisted_data["execution_status"] == "running"

        # Reload the state (simulating a restart after crash)
        loaded_state = ConversationState.create(
            workspace=LocalWorkspace(working_dir="/tmp"),
            persistence_dir=persist_path_for_state,
            agent=agent,
        ...
        # The execution_status should now be ERROR, not RUNNING
        assert loaded_state.execution_status == ConversationExecutionStatus.ERROR
