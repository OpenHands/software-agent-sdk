...
class TestEventsToMessages:
    ...
    def test_malformed_tool_call_args_produce_valid_json_in_history(self):
        """Tool calls with malformed JSON args must serialize to valid JSON.

        Regression test for #2887: llama-server rejects prompts containing
        tool calls with invalid JSON arguments in conversation history.
        """
        malformed_args = '{"command":"create","path":"/tmp/x","file_text":"unterminated'
        bad_tool_call = MessageToolCall(
            id="call_bad",
            name="file_editor",
        ...
        action_event = ActionEvent(
            source="agent",
            thought=[TextContent(text="Creating file")],
            action=None,
            tool_name="file_editor",
            tool_call_id="call_bad",
            tool_call=bad_tool_call,
            llm_response_id="resp_1",
        )
        error_event = AgentErrorEvent(
            error="Error validating args: Unterminated string",
            tool_call_id="call_bad",
            tool_name="file_editor",
        )

        msgs = LLMConvertibleEvent.events_to_messages([action_event, error_event])
        assert len(msgs) == 2

        # The assistant message must serialize tool_calls with valid JSON
        ...
        # Must be valid JSON — this is what llama-server would try to parse
