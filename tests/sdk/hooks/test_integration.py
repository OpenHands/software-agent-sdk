...
""")
    ...
    def test_working_dir_hooks_merged_with_explicit_config(self, tmp_path):
        """Test that working directory hooks are merged with explicit hook_config."""
        import json
        from unittest.mock import patch
        ...
        # Create .openhands/hooks.json in the working directory with a stop hook
        openhands_dir = tmp_path / ".openhands"
        openhands_dir.mkdir()

        working_dir_log = tmp_path / "working_dir_hook.log"
        hooks_dir = openhands_dir / "hooks"
        hooks_dir.mkdir()
        working_dir_script = hooks_dir / "on_stop.sh"
        working_dir_script.write_text(f"""#!/bin/bash
echo "Working dir stop hook called" >> {working_dir_log}
echo '{{"decision": "allow"}}'
exit 0
""")
        working_dir_script.chmod(0o755)
        ...
        # Create an explicit hook_config with a session_start hook
        explicit_log = tmp_path / "explicit_hook.log"
        explicit_script = tmp_path / "explicit_hook.sh"
        explicit_script.write_text(f"""#!/bin/bash
echo "Explicit session_start hook called" >> {explicit_log}
exit 0
""")
        explicit_script.chmod(0o755)

        explicit_config = HookConfig.from_dict(
            {
                "session_start": [
                    {
                        "matcher": "*",
        ...
        with patch.object(Agent, "step", mock_step):
            # Pass explicit hook_config - working dir hooks should ALSO be loaded
            conversation = LocalConversation(
                agent=agent,
                workspace=tmp_path,
                callbacks=[],
                visualizer=None,
                max_iteration_per_run=10,
                hook_config=explicit_config,
            )

            # Send a message to start (triggers plugin/hook loading)
            conversation.send_message("Hello")

            # Verify hook processor was created
            assert conversation._hook_processor is not None
        ...
        # Verify BOTH hooks were called - explicit AND working directory
        assert explicit_log.exists(), "Explicit hook should have been called"
        assert "Explicit session_start hook called" in explicit_log.read_text()

        assert working_dir_log.exists(), "Working dir hook should have been called"
        assert "Working dir stop hook called" in working_dir_log.read_text()
