...
# ---------------------------------------------------------------------------
# complete() / acomplete() unified dispatch tests
# ---------------------------------------------------------------------------


@patch("openhands.sdk.llm.llm.litellm_completion")
def test_complete_dispatches_to_completion_when_not_responses_api(mock_completion):
    """complete() should route to completion() when uses_responses_api() is False."""
    mock_completion.return_value = create_mock_response("from completion")
    llm = LLM(
        usage_id="test-llm",
        model="gpt-4o",
        api_key=SecretStr("test_key"),
...
def test_complete_dispatches_to_responses_when_responses_api(mock_responses):
    """complete() should route to responses() when uses_responses_api() is True."""
    from tests.sdk.llm.test_subscription_mode import _make_responses_api_response

    mock_responses.return_value = _make_responses_api_response("from responses")
    llm = LLM(
...
@patch("openhands.sdk.llm.llm.litellm_acompletion", new_callable=AsyncMock)
async def test_acomplete_dispatches_to_acompletion_when_not_responses_api(
    mock_acompletion,
):
    """acomplete() should route to acompletion() when uses_responses_api() is False."""
    mock_acompletion.return_value = create_mock_response("from acompletion")
    llm = LLM(
        usage_id="test-llm",
        model="gpt-4o",
        api_key=SecretStr("test_key"),
...
async def test_acomplete_dispatches_to_aresponses_when_responses_api(
    mock_aresponses,
):
    """acomplete() should route to aresponses() when uses_responses_api() is True."""
    from tests.sdk.llm.test_subscription_mode import _make_responses_api_response

    mock_aresponses.return_value = _make_responses_api_response("from aresponses")
    llm = LLM(
        model="openai/gpt-5.2-codex",
        base_url="https://chatgpt.com/backend-api/codex",
        reasoning_effort="high",
