...
def create_marketplace(
    ...
    Args:
        ...
        plugins: List of plugin definitions with 'name' and optional 'description'
...
class TestMarketplaceRegistryIntegration:
    ...
    def test_marketplace_caching(self, tmp_path):
        ...
        registry.resolve_plugin("plugin-a@cached")

        # Check that marketplace is now cached
        assert "cached" in registry._cache

        # Second resolution - should use cache
        registry.resolve_plugin("plugin-b@cached")
...
class TestConversationLoadPlugin:
    ...
    def test_load_plugin_from_marketplace(self, tmp_path):
        ...
        expected_source = f"{marketplace_dir}/plugins/test-plugin"
        assert conversation.resolved_plugins[0].source == expected_source
