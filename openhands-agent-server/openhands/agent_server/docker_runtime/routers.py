...
async def _acquire_or_404(
    registry: DockerConversationRegistry, conversation_id: UUID
) -> RunningConversationContainer:
    try:
        return await registry.acquire(conversation_id)
...
async def _proxy_to_conversation(
    registry: DockerConversationRegistry,
    conversation_id: UUID,
    request: Request,
    upstream_path: str,
) -> StreamingResponse:
    container = await _acquire_or_404(registry, conversation_id)
    return await proxy_http(
        request,
        container,
        upstream_path=upstream_path,
        on_complete=lambda: registry.release(conversation_id),
    )
...
async def docker_proxy_conversation_root_mutation(
    ...
    """
    registry = get_registry(request)
    return await _proxy_to_conversation(
        registry,
        conversation_id,
        request,
        _build_upstream_path(request, f"/api/conversations/{conversation_id}"),
    )
...
async def docker_proxy_conversation_subpath(
    ...
    return await _proxy_to_conversation(
        registry, conversation_id, request, upstream_path
    )
...
async def docker_events_websocket(
    ...
    if registry is None:
        ...
        return
    try:
        container = await registry.acquire(conversation_id)
    except KeyError:
    ...
    try:
        await bridge_websocket(websocket, container, upstream_path=upstream_path)
    finally:
...
def _make_docker_global_handler(prefix: str):
    ...
    async def _handler(
        ...
        return await _proxy_to_conversation(registry, cid, request, upstream_path)
