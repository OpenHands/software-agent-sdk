...
class ConversationService:
    ...
    async def get_conversation(self, conversation_id: UUID) -> ConversationInfo | None:
        ...
        if event_service is None:
            persisted = await self._get_persisted_conversation_info(
                conversation_id, include_acp=False
            )
            if persisted is None:
                return None
            assert isinstance(persisted, ConversationInfo)
            return persisted
    ...
    async def get_acp_conversation(
        ...
        if event_service is None:
            persisted = await self._get_persisted_conversation_info(
                conversation_id, include_acp=True
            )
            if persisted is None:
                return None
            assert isinstance(persisted, ACPConversationInfo)
            return persisted
        state = await event_service.get_state()
        ...
        *,
        ...
        for conversation_dir in self._iter_persisted_conversation_dirs():
            ...
            conversation_info = await self._get_persisted_conversation_info(
                conversation_id, include_acp=include_acp
            )
    ...
    async def _get_persisted_conversation_info(
        ...
        *,
        include_acp: bool,
    ) -> ConversationInfo | ACPConversationInfo | None:
    ...
    async def _start_conversation(
        ...
        if (
            persisted_stored is not None
            and not use_acp_contract
            and not _is_v1_conversation(persisted_stored)
        ):
