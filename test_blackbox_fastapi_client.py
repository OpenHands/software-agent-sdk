...
def test_bug_5205():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        response = client.post(
            "/api/agent-profiles/codex-blackbox-test", json=profile_data
        )
        ...
        assert response.status_code == 200, (
            f"Failed to activate profile: {response.text}"
        )
        ...
        assert response.status_code == 200, (
            f"Failed to set agent_settings: {response.text}"
        )
        ...
        assert agent_settings["agent_kind"] == "acp", (
            "agent_settings.agent_kind should be 'acp'"
        )
        assert agent_settings.get("acp_server") == "codex", (
            "agent_settings.acp_server should be 'codex'"
        )
        assert agent_settings.get("acp_model") == "gpt-5.5", (
            "agent_settings.acp_model should be 'gpt-5.5'"
        )
        ...
        print(
            "    WITHOUT FIX: agent_settings would remain with agent_kind='acp' (BUG)"
        )
        ...
        print(
            f"  active_agent_profile_id: {settings_after.get('active_agent_profile_id')}"
        )
