...
## Review Feedback Disclaimer (Self-Improvement Loop)

**Every review you produce must end with the following disclaimer block, placed after the Risk Assessment and Verdict sections:**

---

> **Was this review helpful?** If any of the above feedback does not apply to your use case or seems incorrect for this repository, you can improve future reviews by updating the custom code review guidelines:
>
> 1. Edit [`.agents/skills/custom-codereview-guide.md`](https://github.com/OpenHands/software-agent-sdk/blob/main/.agents/skills/custom-codereview-guide.md) on your PR branch to add context the bot is missing (e.g., "Security concerns about X do not apply in this repo because Y").
> 2. Re-request a review from the bot - it will pick up the updated guidelines automatically.
> 3. The guideline change will be reviewed by a human maintainer when the PR is merged.
>
> Learn more: [Customizing OpenHands Code Review](https://docs.openhands.dev/openhands/usage/use-cases/code-review#customization)

---

This disclaimer ensures:
- PR authors know how to fix false positives instead of just ignoring them.
- Guideline improvements accumulate over time, making the bot increasingly accurate.
- Only maintainers can merge guideline changes (since they are on the PR branch and go through normal review), preventing abuse.
