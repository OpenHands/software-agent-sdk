...
def _http(method: str, url: str, key: str, body: dict | None = None) -> tuple[int, float]:
...
def phase2_http(base_url: str, key: str, concurrency: int = 10, rounds: int = 3) -> int:
    ...
    """
    _banner(f"Phase 2 — HTTP concurrent read load ({concurrency} workers × {rounds} rounds)")
    ...
    def one_cycle(i: int) -> bool:
        ...
        code, t = _http(
            "GET", f"{base_url}/api/conversations/search?limit=5", key
        )
        latencies.append(t)
        total += 1
        if code != 200:
    ...
    for r in range(rounds):
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as pool:
            futs = [pool.submit(one_cycle, r * concurrency + i) for i in range(concurrency)]
