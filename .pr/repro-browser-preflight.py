...
if "--version" in sys.argv:
    ...
    sys.stderr.write("error while loading shared libraries: libstdc++.so.6: cannot open shared object file\\n")
    sys.exit(1)
else:
...
print(f"[repro] --version → exit 1 (missing libstdc++.so)")
print(f"[repro] launch → hangs 300s (C-level block)")
print()
...
if not has_fix:
    ...
    print("[before] OLD: check_chromium_available() → binary found, proceeding to launch")
    print("[before]   In production: BrowserSession.start() dispatches BrowserStartEvent")
    print("[before]   → handler hangs on C-level launch, timeout fires but thread never freed")
    print()
    ...
    if done.wait(timeout=6):
        ...
        if rc is not None:
            ...
            print(f"[before]   process still running after 5s")
    else:
        print(f"[before]   *** HANG *** — browser didn't respond after 5s")
        print(f"[before]   In production, the bubus handler timeout fires after 30s")
        print(f"[before]   but the thread is never freed → zombie thread accumulates")

    proc.kill()
    zombies = [t for t in threading.enumerate() if t.is_alive() and not t.daemon and t is not threading.main_thread()]
    ...
    try:
        ...
        print(f"[after] RESULT: PASS — pre-flight passed in {elapsed:.1f}s (unexpected)")
    except Exception as e:
        elapsed = time.monotonic() - t0
        msg = str(e)[:200]
        print(f"[after] RESULT: FAIL FAST in {elapsed:.1f}s")
        print(f"[after]   error: {msg}")
        print(f"[after]   No bubus event dispatched, no zombie thread")
