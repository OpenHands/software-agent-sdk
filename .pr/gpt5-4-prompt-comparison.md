...
| `<OUTPUT_VERBOSITY_SPEC>` | `gpt_5_2_prompt.md` `Personality`; `gpt_5_codex_prompt.md` `Presenting your work and final message` | Codex has broad style rules; OpenHands compresses that into a compact default-answer-size policy. This is a transpilation into our existing response style instead of a line-by-line port. |
...
| `<PRESENTING_WORK>` | `gpt_5_codex_prompt.md` `Presenting your work and final message` | The "lead with what changed" and "reference paths instead of asking the user to save/copy" ideas are direct adaptations. The original unconditional "user is on the same machine" assumption is now gated by `cli_mode`, because OpenHands can also run remotely. |
...
- GPT-5 preamble behavior lives in `openhands-sdk/openhands/sdk/agent/prompts/model_specific/openai_gpt/gpt-5.j2`
